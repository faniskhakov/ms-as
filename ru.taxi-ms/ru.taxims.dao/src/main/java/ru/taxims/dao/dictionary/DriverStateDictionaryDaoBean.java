...
import ru.taxims.domain.datamodels.DriverState;
...
public class DriverStateDictionaryDaoBean extends DictionaryDaoBean<DriverState>
{
	@PostConstruct
	public void init() throws DaoException
	{
		...
		try {
			...
			preparedStatement = connection.prepareStatement("SELECT state_id, \"name\" FROM driver_state ");
			result = preparedStatement.executeQuery();
			while (result.next()) {
				int stateId = result.getInt("state_id");
				DriverState state = new DriverState(stateId);
	...
	private List<Integer> getAvailableStates(int stateId, Connection connection) throws SQLException
	{
		...
		PreparedStatement preparedStatement = connection.prepareStatement("SELECT state_id, old_state_id " +
			"  FROM driver_state_transition_rule " +
			" WHERE old_state_id = ?" +
	...
	public int insertDriverState(DriverState state, List<Integer> availableStates) throws DaoException
	{
		assert dataSource != null;
		Connection connection = null;
		PreparedStatement preparedStatement = null;
		...
		try {
			connection = dataSource.getConnection();
			String INSERT_DRIVER_STATE_SQL = "INSERT INTO driver_state " +
				" (\"name\") " +
				" VALUES (?) RETURNING state_id ";
			preparedStatement = connection.prepareStatement(INSERT_DRIVER_STATE_SQL);
			...
			if (generatedKeys.next()) {
				...
				throw new DaoException("Inserting driver state failed, no generated key obtained.");
			}

			String INSERT_DRIVER_STATE_CHANGE_RULE_SQL = "INSERT INTO driver_state_transition_rule(\n " +
				"	state_id, old_state_id)\n " +
				" VALUES (?, ?)";
			preparedStatement = connection.prepareStatement(INSERT_DRIVER_STATE_CHANGE_RULE_SQL);
