...
public class TransactionStateDictionaryDaoBean extends DictionaryDaoBean<TransactionState>
{
	@PostConstruct
	public void init() throws DaoException
	{
		...
		try {
			...
			preparedStatement = connection.prepareStatement("SELECT state_id, \"name\" FROM transaction_state ");
			result = preparedStatement.executeQuery();
			while (result.next()) {
				int stateId = result.getInt("state_id");
				TransactionState state = new TransactionState(stateId);
	...
	private List<Integer> getAvailableStates(int stateId, Connection connection) throws SQLException
	{
		...
		PreparedStatement preparedStatement = connection.prepareStatement("SELECT state_id, old_state_id " +
			"  FROM transaction_state_transition_rule " +
			" WHERE old_state_id = ?" +
	...
	public int insertTransactionState(TransactionState state, List<Integer> availableStates) throws DaoException
	{
		assert dataSource != null;
		Connection connection = null;
		PreparedStatement preparedStatement = null;
		...
		try {
			connection = dataSource.getConnection();
			String INSERT_ORDER_STATE_SQL = "INSERT INTO transaction_state " +
				" (\"name\") " +
			...
			if (generatedKeys.next()) {
				...
				throw new DaoException("Inserting transaction state failed, no generated key obtained.");
			}

			String INSERT_ORDER_STATE_CHANGE_RULE_SQL = "INSERT INTO transaction_state_transition_rule(\n " +
